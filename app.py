...
# Global list to store manually selected points
selected_points = []

def draw_markers(image, points):
    """Draws numbered markers on the image at specified points."""
    if image is None:
        return None
    img_copy = image.copy()
...
def select_point(img, evt: gr.SelectData):
    """Callback to collect clicked points and mark them on the image."""
...
def reset_points():
    ...
    global selected_points
    selected_points = []
    return None  # Clears the image

def process_image(image):
    """Processes the image if 4 manually selected points are provided."""
    if len(selected_points) != 4:
...
def auto_detect(image):
    """Automatically detects corner points, draws markers, and processes the image."""
    processor = ProcessorFactory.create_processor()
...
def main():
    with gr.Blocks(title="App") as demo:
        ...
        # Buttons for processing, auto-detect and resetting points
